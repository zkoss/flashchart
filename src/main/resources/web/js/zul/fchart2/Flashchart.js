...
(function () {
	...
	zk.afterMount(function () {
		...
		if (!YAHOO.widget)
			YAHOO.widget = {};
		if (!YAHOO.widget.FlashAdapter)
			YAHOO.widget.FlashAdapter = {};

		var oldHandler = YAHOO.widget.FlashAdapter.eventHandler;
		YAHOO.widget.FlashAdapter.eventHandler = function (id, event) {
			if (id && event.type == "swfReady") {
				for (var nm in _caches) {
					if (_caches[nm] == id) {
						var wgt = zk.Widget.$(nm);
						if (wgt) {
							zul.fchart2.Flashchart.onEvent(nm, event);
							break;
						}
					}
				}
	...
	var _src = zk.ajaxURI('/web/js/zul/fchart2/charts.swf', {au: true}),
		_expressInstall = zk.ajaxURI('/web/js/zul/fchart2/expressinstall.swf', {au: true}),
		_axis = {
 ...
 *
 */
zul.fchart2.Flashchart = zk.$extends(zul.med.Flash, {
	...
	bind_: function (desktop, skipper, after) {
		...
		var _swfId = this.uuid + "-chart",
			_flashvars = "allowedDomain=" + (document.domain || 'localhost') + "&elementID=yuigen" + yuigen + 
				"&eventHandler=zul.fchart2.Flashchart.onEvent",
			_params = {				
		...
		zul.fchart2.swfobject.embedSWF(src, _swfId, this._width, this._height, 
